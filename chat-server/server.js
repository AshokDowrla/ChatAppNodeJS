...
const io = new Server(server, {
  cors: {
    origin: "https://tele-postletters.onrender.com",
    methods: ["GET", "POST"],
  },
});
...
io.on("connection", async (socket) => {
  console.log(JSON.stringify(socket.handshake.query), "Socket Handshake Query");
  ...
  socket.on("get_direct_conversations", async ({ user_id }, callback) => {
    ...
    console.log(existing_conversations, "get_direct_conversations");
  ...
  socket.on("start_conversation", async (data) => {
    ...
    if (existing_conversations?.length === 0) {
      ...
      new_chat = await OneToOneMessage.findById(new_chat).populate(
        "participants",
        "firstName lastName _id email status avatar"
      );
  ...
  socket.on("delete_message", async (data) => {
    const chat = await OneToOneMessage.findById(data.room_id).select(
      "messages"
    );
    const { messages } = chat;
    const filteredMsgs = messages.filter(
      (ele) => ele._id.toString() !== data.msg_id
    );

    const deletedMsg = messages.filter(
      (ele) => ele._id.toString() === data.msg_id
    );

    chat.messages = [...filteredMsgs];

    await chat.save({
      new: true,
      validateModifiedOnly: true,
    });
    // console.log(filteredMsgs, deletedMsg[0]);
    // emit updated msggs to both users

    const to_user = await User.findById(deletedMsg[0].to);
    const from_user = await User.findById(deletedMsg[0].from);

    io.to(to_user?.socket_id).emit("update_messages", {
      room_id: data.room_id,
      filteredMsgs,
    });

    io.to(from_user?.socket_id).emit("update_messages", {
      room_id: data.room_id,
      filteredMsgs,
    });
  ...
  socket.on("text_message", async (data) => {
    ...
    // save to db`
    const saved_msg = await chat.save({
      new: true,
      validateModifiedOnly: true,
    });
    // console.log(saved_msg.messages);
    const latest_msg = saved_msg.messages.pop();
  ...
  socket.on("end", async (data, logOutFE) => {
